describe("serialize", function() {
    ...
    it("serializes DOM nodes", function() {
        ...
        node.name = "foo";
        node.value = "bar";
        expect(ugma.serialize(node)).toEqual({ foo: "bar" });

        node.name = "";
        expect(ugma.serialize(node)).toEqual({});
    });

    it("retuns empty object for invalid elements", function() {
        expect(ugma.serialize(document.documentElement)).toEqual({});
    });
  ...
  function testForm(html, value) {
        it(html, function() {
            ...
            expect(ugma.serialize(form)).toEqual(value);
        });
    }
